...
summary: Trident 운영자를 수동으로 배포하여 Trident를 설치할 수 있습니다. 이 프로세스는 Trident에 필요한 컨테이너 이미지가 프라이빗 레지스트리에 저장되지 않은 설치에 적용됩니다. 
---
...
[role="lead"]
Trident 운영자를 수동으로 배포하여 Trident를 설치할 수 있습니다. 이 프로세스는 Trident에 필요한 컨테이너 이미지가 프라이빗 레지스트리에 저장되지 않은 설치에 적용됩니다. 프라이빗 이미지 레지스트리가 있는 경우 link:kubernetes-deploy-operator-mirror.html["오프라인 배포 프로세스"]을(를) 사용하십시오.
...
이제  `TridentOrchestrator`를 생성하고 Trident를 설치할 수 있습니다. 선택적으로, link:kubernetes-customize-deploy.html["Trident 설치를 사용자 지정합니다"]를  `TridentOrchestrator` 사양의 속성을 사용하여 수행할 수 있습니다.
