...
summary: protectctl 명령을 사용하여 Trident Protect 사용자 지정 리소스를 생성하고 상호 작용할 수 있습니다. 
---
...
--
.단계
. Trident Protect CLI 플러그인을 다운로드합니다.
+
[source, console]
----
...
----
. PATH 변수에 정의된 위치에 플러그인 바이너리를 복사합니다. 예를 들어 `/usr/bin` 또는 `/usr/local/bin` (상승된 권한이 필요할 수 있음):
+
[source, console]
----
cp ./tridentctl-protect /usr/local/bin/
----
. 선택적으로 플러그인 바이너리를 홈 디렉토리의 위치에 복사할 수 있습니다. 이 경우 해당 위치가 PATH 변수의 일부인지 확인하는 것이 좋습니다:
+
[source, console]
----
...
NOTE: PATH 변수의 위치에 플러그인을 복사하면 어느 위치에서나 `tridentctl-protect` 또는 `tridentctl protect`을 입력하여 플러그인을 사용할 수 있습니다.
...
----
. 다운로드한 스크립트를  `~/.bash/completions` 디렉터리로 이동합니다.
+
[source, console]
----
mv tridentctl-completion.bash ~/.bash/completions/
----
. 홈 디렉토리의 `~/.bashrc` 파일에 다음 줄을 추가합니다.
+
[source, console]
----
...
----
. 다운로드한 스크립트를  `~/.zsh/completions` 디렉토리로 이동합니다.
+
[source, console]
----
mv tridentctl-completion.zsh ~/.zsh/completions/
----
. 홈 디렉토리의 `~/.zprofile` 파일에 다음 줄을 추가합니다.
+
[source, console]
----
